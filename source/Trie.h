...
void Trie::addWord(string start, string pred, unsigned int freq)
{
	...
	{        
		...
		if ( childNode != NULL )
		{
			...
			//overwrites predictions if new word's freq is higher
			//if (currentNode->getContent().getFreq() < newWord.getFreq())
...
void Trie::appendWord(ValidChar newWord)
{
	...
	{        
		...
		{
			currentNode = childNode;
			if ((i == newWord.getString ().length () - 1) && /* this is added*/ (currentNode->getContent().getString() == "") )
				currentNode->getContentUnprotected().updatePred(newWord.getString(),newWord.getFreq());
		}
		else
		{
